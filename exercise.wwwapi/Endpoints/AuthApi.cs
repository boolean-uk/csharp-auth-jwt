...
namespace exercise.wwwapi.Endpoints
{
    public static class AuthApi
    {
        ...
        public static async Task<IResult> Login(UserRequestDTO request, IRepository<User> repo, IConfigurationSettings config)
        {
            if (!repo.GetAll().Where(u => u.Username == request.UserName).Any()) return Results.BadRequest(new Payload<UserRequestDTO>() { status = "User does not exist", data = request });

            User user = repo.GetAll().FirstOrDefault(u => u.Username == request.UserName);
            ...
            return Results.Ok(new Payload<string>() { status = "success", data = token});
        }
