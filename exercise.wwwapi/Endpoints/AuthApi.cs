...
namespace exercise.wwwapi.Endpoints
{
    public static class AuthApi
    {
        ...
        private static async Task<IResult> Register(UserRequestDTO request, IRepository<User> service)
        {
            ...
            //user exists
            if (service.GetAll().Result.Where(u => u.Username == request.Username).Any()) return Results.Conflict(new Payload<UserRequestDTO>() { status = "Username already exists!", data = request });
        ...
        {
            //user doesn't exist
            if (!service.GetAll().Result.Where(u => u.Username == request.Username).Any()) return Results.BadRequest(new Payload<UserRequestDTO>() { status = "User does not exist", data = request });

            User user = service.GetAll().Result.FirstOrDefault(u => u.Username == request.Username)!;
